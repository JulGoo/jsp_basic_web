			...
			<div class="row">
				<table class="table table-striped table-hover">
					...
					<tbody>
						...
						ArrayList<Bbs_hDTO> list = bbs.getCategoryList(pageNumber, "etc");
						
						int numbering = 0;
						for (int i = 0; i < list.size(); i++) {
							int count = bbs.listofcategoryCount("etc");
							if(pageNumber != 1){
				...
				if(pageNumber != 1) {
				%>
				<a href="bbs_etc.jsp?pageNumber=<%= pageNumber - 1 %>" class="btn btn-light">이전</a>
				<%
				} if(bbs.nextPage(pageNumber + 1, "etc")) {
				%>
				<a href="bbs_etc.jsp?pageNumber=<%= pageNumber + 1 %>" class="btn btn-light">다음</a>
				<%
				}
				%> 				
				<br>
