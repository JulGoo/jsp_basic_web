...
public class Bbs_hDAO {
	...
	public boolean nextPage(int pageNumber) {
		String SQL = "select * from bbs_h where no < ? and available = 1";
		try {
			PreparedStatement pstmt = conn.prepareStatement(SQL);
			pstmt.setInt(1,  getNext() - (pageNumber - 1) * 10);
			ResultSet rs = pstmt.executeQuery();
	...
	//게시글 수정 메소드
	public int update(int no, String rest, String title, String content) {
		String SQL = "update bbs_h set rest = ?, title = ?, content = ? where no = ?";
		try {
			PreparedStatement pstmt = conn.prepareStatement(SQL);
